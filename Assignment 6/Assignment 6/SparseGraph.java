...
public class SparseGraph<V, E> implements Graph<V, E> {
    ...
    public SparseVertex<V> convert(Vertex<V> invertex)
        ...
        try {
            ...
            throw new PositionException();
        } //bitwise operator, exception will be called error regardless
    }
    ...
    public String toString() {
        ...
        s += "digraph {\n";
        for (SparseVertex<V> vertices : incidenceList) {
            s += "  \"" + convert(vertices).data + "\";\n";
        }
        for (SparseVertex<V> vertices: incidenceList) {
            for (SparseEdge<E> edges : vertices.out) {
                s += " \"" + convert(edges.previous).data + "\"" + " -> \""
                        + convert(edges.next).data + "\" "
                        + convert(edges).data;
            }
        }
        s += "}";
